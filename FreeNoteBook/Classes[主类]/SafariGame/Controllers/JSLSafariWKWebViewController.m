...
{
    return self.htmlUrlString;
}

#pragma mark 根据网址加载网页
- (void)webViewStartLoadRequest
{
    NSString *requestUrl = [self webViewLoadRequestURLStrring];
    JSLLog(@"刷新请求地址=>%@",requestUrl);
    
    if (![URL_DEFAULT_NETWORK_ABAILABLE isEqualToString:requestUrl]) {
        ...
        // 请求网络地址数据
        [self requestNetworkURLStringDataThen:^(BOOL success, JSLSafariUrlModel *safariUrlModel) {
            
            // 删除错误提示页面
            [weakSelf.webErrorView removeFromSuperview];
            [weakSelf setWebErrorView:nil];
            // 显示加载动画菊花
            [self.activityIndicatorView startAnimating];
            
            // 重新加载请求
            NSString *requestUrl = [weakSelf webViewLoadRequestURLStrring];
            NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:requestUrl]];
            [request setTimeoutInterval:30.0f];
...
{
    [JSLNetworkHTTPSessionUtil POST:URL_API_BASE parameters:nil success:^(id responseObject) {
        
        JSLLog(@"获取数据成功 = %@", responseObject);
        
        JSLSafariUrlModel *safariUrlModel = [JSLSafariUrlModel mj_objectWithKeyValues:responseObject];
        
        // 操作处理
        !then ?: then(YES, safariUrlModel);
...
- (void)viewDidLoad
{
    ...
    {
        // 初始化progressView
        self.progressView = [[JSLProgressView alloc] initWithFrame:CGRectMake(0, STATUS_BAR_HEIGHT, SCREEN_WIDTH, JSL_AUTOSIZING_WIDTH(1.5))];
        // 设置未走过进度的进度条颜色
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"title"]) {
        ...
        self.progressView.progress = (self.webView.estimatedProgress <= 0.2f) ? 0.2f : self.webView.estimatedProgress;
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error
{
    ...
    // 加载失败显示错误信息
    {
        ...
        CGRect frame = CGRectMake(0,
                                  STATUS_BAR_HEIGHT,
                                  SCREEN_WIDTH,
                                  SCREEN_HEIGHT-STATUS_BAR_HEIGHT-TAB_BAR_AND_DANGER_HEIGHT);
        JSLWKWebErrorView *webErrorView = [[JSLWKWebErrorView alloc] initWithFrame:frame];
        [webErrorView setRefreshBlock:^{
            [weakSelf pressButtonWebViewErrorRefreshAction];
        }];
...
- (WKWebViewConfiguration *)config
{
    if (!_config) {
        ...
        // _config.mediaPlaybackRequiresUserAction = YES; // 设置是否允许视屏自动播放
        // iOS系统版本 >= 9.0
        if (@available(iOS 9.0, *)) {
...
- (DGActivityIndicatorView *)activityIndicatorView
{
    if (!_activityIndicatorView) {
        CGFloat activityIndicatorSize = JSL_AUTOSIZING_WIDTH(60.0f);
        ...
        [_activityIndicatorView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerY.equalTo(self.view.mas_centerY).offset(activityIndicatorSize/2.0f);
        }];
    }
...
#pragma mark 页面内容是否允许缩放
- (void)setWebViewZoomEnable:(BOOL)enable
{
    if (enable) {
        return;
    }
    ...
    [self.webView evaluateJavaScript:injectionJSString completionHandler:^(id _Nullable result, NSError * _Nullable error) {
