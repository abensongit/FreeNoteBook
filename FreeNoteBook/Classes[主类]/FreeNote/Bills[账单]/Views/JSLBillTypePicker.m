...
@interface JSLBillTypePicker() <JSLToolbarDelegate, UIPickerViewDelegate, UIPickerViewDataSource>

@property (nonatomic, strong) UIView *bgView;
@property (nonatomic, strong) UIView *tapView;
@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) JSLToolbar *toolbar;
@property (nonatomic, strong) UIPickerView *pickerView;
@property (nonatomic, strong) NSArray *typeItems;

@end

@implementation JSLBillTypePicker
...
- (JSLToolbar *)toolbar
{
    if (!_toolbar) {
        CGRect rect = [UIApplication sharedApplication].delegate.window.bounds;
        _toolbar = [[JSLToolbar alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, 43) leftButtonTitle:NSLocalizedString(@"取消", nil) middleLabel:NSLocalizedString(@"选择类型", nil) rightButtonTitle:NSLocalizedString(@"确定", nil)];
...
{
    if (!_typeItems) {
        JSLBillTypeInfo *payInfo = [[JSLBillTypeInfo alloc] init];
        ...
        JSLBillTypeInfo *incomeInfo = [[JSLBillTypeInfo alloc] init];
...
#pragma mark - JSLToolbarDelegate
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        JSLBillTypeInfo *info = self.typeItems[selRow];
...
{
    if (component == 0) {
        JSLBillTypeInfo *info = self.typeItems[row];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    JSLBillTypeInfo *info = self.typeItems[typeRow];
