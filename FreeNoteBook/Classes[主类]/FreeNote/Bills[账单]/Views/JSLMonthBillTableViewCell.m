...
#import "PNChart.h"
#import "JSLMonthBillLegend.h"

@interface JSLMonthBillTableViewCell()
...
@property (nonatomic, strong) JSLMonthBillLegend *legend;
@property (nonatomic, strong) UIImageView *picImageView;

@end

@implementation JSLMonthBillTableViewCell
...
- (UIView *)dividerView
{
    if (!_dividerView) {
        ...
        _dividerView.backgroundColor = [JSLThemeConfig currentTheme].mainBGColor;
    }
...
- (UIView *)dividerLine
{
    if (!_dividerLine) {
        ...
        _dividerLine.backgroundColor = [JSLThemeConfig currentTheme].dividerColor;
    }
...
- (JSLMonthBillLegend *)legend
{
    if (!_legend) {
        _legend = [[JSLMonthBillLegend alloc] initWithFrame:CGRectMake(0, 0, 100, 80)];
    }
...
- (void)updateCellWithTitle:(NSString *)title monthInfo:(JSLBillMonthInfo *)info
{
    ...
    if (items.count > 0) {
        ...
        if ([title isEqualToString:NSLocalizedString(@"总收入", nil)]) {
            self.picImageView.image = [UIImage imageNamed:ICON_BILL_MONTH_RECORD_IN_COME_NO];
        } else {
            self.picImageView.image = [UIImage imageNamed:ICON_BILL_MONTH_RECORD_PAY_NO];
        }
    }
}

- (NSArray *)buildDataItems:(JSLBillMonthInfo *)monthInfo type:(NSString *)type
{
