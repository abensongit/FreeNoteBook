...
@interface JSLTips (KeyboardFrame)
...
@interface JSLTipsView()

@property (nonatomic, copy) JSLTipsInfo      *tipsInfo;
...
//        }f
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(handleKeyboardFrameChange:)
                                                   ...
                                                   object:nil];
        _keyboardFrame = [JSLTips sharedTips].keyboardFrame;
...
- (void)executeAnimation:(JSLTipsInfo *)tips completion:(void (^)(void))block
{
    self.tipsInfo = tips;
...
- (void)animateAppear:(void (^)(void))completeBlock
{
    ...
    if (self.tipsInfo.showAnimate == JSLTipsAnimationFlat) {
        self.alpha = 0;
...
{
    if (self.tipsInfo.hideAnimate == JSLTipsAnimationFlat) {
...
- (void)processTipsPosition
{
    ...
    if (self.tipsInfo.position == JSLTipsPositionTop) {
    ...
    else if (self.tipsInfo.position == JSLTipsPositionBottom) {
