...
@interface JSLTextField()<UITextFieldDelegate>
...
- (void)moveAnimation:(CGFloat)x y:(CGFloat)y
{
    ...
    [UIView animateWithDuration:0.15 animations:^{
        moveY -= self->_placeholderLabel.frame.size.height/2 + heightSpaceing;
        moveX -= padding;
        self->_placeholderLabel.center = CGPointMake(moveX, moveY);
        self->_placeholderLabel.alpha = 1;
        self->_moved = YES;
        self->_lineLayer.bounds = CGRectMake(0, 0, CGRectGetWidth(self.frame), lineWidth);
    }];
}
...
- (void)backAnimation:(CGFloat)x y:(CGFloat)y
{
    ...
    [UIView animateWithDuration:0.15 animations:^{
        ...
        self->_placeholderLabel.alpha = 1;
        self->_moved = NO;
        self->_lineLayer.bounds = CGRectMake(0, 0, 0, lineWidth);
    }];
}
