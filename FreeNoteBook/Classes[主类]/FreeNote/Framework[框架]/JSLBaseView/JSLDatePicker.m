...
@interface JSLDatePicker() <JSLToolbarDelegate>
...
{
    if (!_tapView) {
        CGFloat height = JSL_AUTOSIZING_HEIGTH_SCALE(216.0f);
        CGRect rect = [UIApplication sharedApplication].delegate.window.bounds;
...
- (UIView *)contentView
{
    if (!_contentView) {
        CGFloat height = JSL_AUTOSIZING_HEIGTH_SCALE(216.0f);
        CGRect rect = [UIApplication sharedApplication].delegate.window.bounds;
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, rect.size.height, rect.size.width, height)];
...
- (UIDatePicker *)datePicker
{
    if (!_datePicker) {
        CGFloat height = JSL_AUTOSIZING_HEIGTH_SCALE(216.0f);
        CGRect rect = [UIApplication sharedApplication].delegate.window.bounds;
        _datePicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, height)];
...
- (JSLToolbar *)toolbar
{
    if (!_toolbar) {
        CGRect rect = [UIApplication sharedApplication].delegate.window.bounds;
        _toolbar = [[JSLToolbar alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, JSL_AUTOSIZING_WIDTH_SCALE(44.0f)) leftButtonTitle:NSLocalizedString(@"取消", nil) middleLabel:NSLocalizedString(@"选择日期", nil) rightButtonTitle:NSLocalizedString(@"确定", nil)];
